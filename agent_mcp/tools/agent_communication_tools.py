...
def _can_agents_communicate(
    sender_id: str, recipient_id: str, is_admin: bool
) -> tuple[bool, str]:
    """
    ...
    # Check if sender is a testing agent and recipient is their assigned agent
    if sender_id.startswith("test-"):
        # Extract task ID from testing agent ID (format: test-{task_id_suffix})
        try:
            # Get the testing agent's database record to find their assigned task
            from ..db.connection import get_db_connection
            ...
            try:
                ...
                # Get the testing agent's current task (which is the completed task they're testing)
                cursor.execute(
                    "SELECT current_task FROM agents WHERE agent_id = ?", (sender_id,)
                )
                result = cursor.fetchone()

                if result:
                    completed_task_id = result["current_task"]

                    # Get the agent who completed that task
                    cursor.execute(
                    ...
                    if task_result and task_result["assigned_to"] == recipient_id:
                        return True, "Testing agent messaging their assigned agent"
...
async def send_agent_message_tool_impl(
    ...
    message_type = arguments.get(
        "message_type", "text"
    )  # text, assistance_request, task_update
    ...
    if not recipient_id or not message_content:
        return [
            mcp_types.TextContent(
                type="text", text="Error: recipient_id and message are required"
            )
        ]
    ...
    if len(message_content) > 4000:  # Reasonable message size limit
        return [
            mcp_types.TextContent(
                type="text", text="Error: Message too long (max 4000 characters)"
            )
        ]
    ...
    if message_type == "stop_command" and not is_admin:
        return [
            mcp_types.TextContent(
                type="text", text="Error: Only admin can send stop commands"
            )
        ]
    ...
    if not can_communicate:
        return [
            mcp_types.TextContent(type="text", text=f"Communication denied: {reason}")
        ]
    ...
    try:
            ...
            if recipient_id in g.agent_tmux_sessions:
                ...
                if session_exists(session_name):
                    ...
                    if message_type == "stop_command":
                        ...
                        try:
                            ...
                            for i in range(4):
                                ...
                                if result.returncode != 0:
                                    ...
                                    break
                                logger.debug(
                                    f"Sent Escape {i+1}/4 to agent {recipient_id}"
                                )
                            ...
                            if success:
                                ...
                                logger.info(
                                    f"Stop command (4x Escape) sent to agent {recipient_id} in session {session_name}"
                                )
                            else:
                                delivery_status = "stop_command_failed"
                                logger.error(
                                    f"Failed to send stop command: {result.stderr}"
                                )

                            # Mark as delivered in database
                            cursor.execute(
                                "UPDATE agent_messages SET delivered = ? WHERE message_id = ?",
                                (success, message_id),
                            )

                        except Exception as e:
                            logger.error(
                                f"Failed to send stop command to tmux session '{session_name}': {e}"
                            )
                            delivery_status = "stop_command_failed"
                    else:
                        ...
                        try:
                            send_prompt_async(
                                session_name, formatted_message, delay_seconds=1
                            )
                            ...
                            logger.error(
                                f"Failed to deliver message to tmux session '{session_name}': {e}"
                            )
        ...
        return [
            mcp_types.TextContent(
                type="text", text=f"Database error sending message: {e}"
            )
        ]
    except Exception as e:
        if conn:
        ...
        return [
            mcp_types.TextContent(
                type="text", text=f"Unexpected error sending message: {e}"
            )
        ]
    finally:
        if conn:
...
async def get_agent_messages_tool_impl(
    ...
    try:
        ...
        if include_received and include_sent:
            ...
            return [
                mcp_types.TextContent(
                    type="text", text="Error: Must include sent or received messages"
                )
            ]
        ...
        if mark_as_read and include_received:
            ...
            if message_ids_to_mark:
                ...
                cursor.execute(
                    f"UPDATE agent_messages SET read = ? WHERE message_id IN ({placeholders})",
        ...
        response_lines = [
            f"Messages for {agent_id} (showing {len(messages)} of max {limit}):"
        ]
        ...
        for msg in messages:
            ...
            response_lines.append(
                f"{direction} {read_status} {priority_icon} [{msg['message_type']}] {other_agent}"
            )
        ...
        return [
            mcp_types.TextContent(
                type="text", text=f"Database error retrieving messages: {e}"
            )
        ]
    except Exception as e:
        ...
        return [
            mcp_types.TextContent(
                type="text", text=f"Unexpected error retrieving messages: {e}"
            )
        ]
    finally:
        if conn:
...
async def broadcast_admin_message_tool_impl(
    ...
    if not active_agents:
        return [
            mcp_types.TextContent(type="text", text="No active agents to broadcast to")
        ]
